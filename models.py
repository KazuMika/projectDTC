...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nC, img_dim, anchor_idxs, cfg):
        ...
        if cfg.endswith('yolov3-tiny.cfg') or cfg.endswith('my-tiny.cfg'):
            stride *= 2
    ...
    def forward(self, p, targets=None, var=None):
        ...
        if targets is not None:
            ...
            tx, ty, tw, th, mask, tcls = build_targets(
                targets, self.anchor_wh, self.nA, self.nC, nG)
...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, var=0):
        ...
        if ONNX_EXPORT:
            # merge the 3 layers 85 x (507, 2028, 8112) to 85 x 10647
...
def test():
    ...
    module_defs[0]['cfg'] = cfg_path
    module_defs[0]['height'] = img_size

    #ss = create_modules(module_defs)
    model = Darknet(cfg_path, 288)
    print(model)
