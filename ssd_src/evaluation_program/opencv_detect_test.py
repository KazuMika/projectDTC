...
parser.add_argument('--frame_rate','-f', default=15.0,type=float, help='sum the integers (default: find the max)')
...
def match_trash2(cords,trash):
    ...
    while ious_all.shape[0] > 0 and ious_all.shape[1] > 0:
        #trash_id_max = np.argmax(ious_all) % len(ious_all[0])
        #cords_id_max = int(np.argmax(ious_all) / len(ious_all))
        ...
        print("shape",ious_all.shape[0]," ",ious_all.shape[1])
        print(type(temp[0]))
        sets.append([trash_ids[temp[1]],temp[0]])
...
def main():
    ...
    net.load_weights('./ssd300_0712_115000.pth')
    if args.test==True:
        avis = "testgomi2.avi"
        ...
        print(avis)
        for avi in avis:
            print(os.path.split(avi))
            path2 = avi.replace(".avi",".mp4").replace("../../","").replace("/","_")
            path2 = "./test/" +path2
            if os.path.exists(path2):
                continue
            make_avi(avi,ni2.aviet,args.frame_rate)
