...
def make_avi(path_to_avi,net, frame_rate,b):
    if not os.path.exists("./test/"+str(b)):
        os.mkdir("./test/"+str(b))
    ...
    video = cv2.VideoWriter("./test/"+str(b) +"/" + path_to_avi, fourcc, frame_rate, (int(cap.get(3)),int(cap.get(4))))
...
def main():
    ...
    if args.test==True:
        ...
        avis = [avi for avi in find_all_files("../../back_kanen_1_8_data/kanen_6/05m11d09h00m") if ".avi" in avi]
        avis.sort()
        ...
        for i in [2,4,6,8]:
            ...
            for j,avi in enumerate(avis):
                ...
                #    continue
                make_avi(avi,net,args.frame_rate,b)
                if j > 7:
                    break
