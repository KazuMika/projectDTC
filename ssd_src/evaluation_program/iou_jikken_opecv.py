...
def match_trash2(cords,trashs,b):
    if len(cords) == 0:
        return None
    if len(trashs) == 0:
...
def make_avi(path_to_avi,net, frame_rate,b,count):
    if not os.path.exists("./iou/"+str(b)):
        os.mkdir("./iou/"+str(b))
    ...
    path_to_avi = path_to_avi.replace(".avi","").replace("../../","").replace("/","_")
    print(path_to_avi)
    #video = cv2.VideoWriter("./test/" + path_to_avi, fourcc, frame_rate, (int(cap.get(3)),int(cap.get(4))))
    #video = cv2.VideoWriter("./iou/"+str(b) +"/" + path_to_avi, fourcc, frame_rate, (int(cap.get(3)),int(cap.get(4))))
    ...
    while(cap.isOpened()):
        ...
        cords, texts = detect_image(rgb_image,net,b)
        cords = bSort(cords)
        detected_trash_ids = []
        sets = match_trash2(cords,trashs,b)
                        ...
                        if i.id==trash_id:
                            ...
                            if i.going_DOWN(line_down) == True:
                                if i.state == True:
                                    ...
                                    cv2.rectangle(frame,(cord[0],cord[1]),(cord[2],cord[3]),(255,0,0),2)
                                    cv2.imwrite("./iou/"+ str(b) + "/{0:02d}_".format(count) + path_to_avi + "_{0:04d}.jpg".format(cnt_down),frame)
                                    i.state = False
                                    i.done = True
...
def main():
    ...
    #net.load_weights('./weights/v2.pth')
    if args.test==True:
        avis = "testgomi2.avi"
        ...
        avis = [avi for avi in find_all_files("../../back_kanen_1_8_data_old/kanen_6/05m11d09h00m") if ".avi" in avi]
        avis.sort()
        avis.insert(8,"testgomi2.avi")
        print(avis)
        for i in [0,0.001,0.005,0.01,0.05,0.1,0.2]:
            b = i 
            net = build_ssd('test', 300, 2)
            ...
            for j,avi in enumerate(avis):
                ...
                #    continue
                make_avi(avi,net,args.frame_rate,b,count)
                if j > 7:
                    break
